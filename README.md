...
| AI 服务接入 | 任意 OpenAI 兼容接口（内置 DeepSeek/Kimi/智谱/通义/OpenAI 预设）：接口地址 / 模型 / 密钥，一键测试连接 |
...
1. **个人中心 → AI 服务接入**（完整表单）
2. **跑步报告** →「配置 AI 服务」链接（无密钥时显示）
3. **连接** →「配置 AI 服务密钥」链接

在「服务商预设」下拉中选择一家一键填充地址与模型，或手动填写（任意 OpenAI 兼容接口均可）。以 DeepSeek 为例：
